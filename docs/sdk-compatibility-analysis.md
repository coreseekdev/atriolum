...
> 对照 Atriolum 当前实现（commit 57f4554）逐项比对
...
| **Rust** | ✅ 基本兼容 | ⚠️ 未实测 | ❌ | 无阻塞问题，有小差异 |
| **JavaScript** | ✅ 基本兼容 | ⚠️ 未实测 | ❌ | 缺少 rate limit 响应头 |
| **C++ (Native)** | ⚠️ 部分兼容 | ❌ 不可用 | ❌ | 缺少 `/api/{id}/minidump/` 端点 |
...
| 优先级 | 问题 | 影响 | 涉及 SDK |
|--------|------|------|----------|
| **P0** | 添加 `/api/{id}/minidump/` 端点 | C++ SDK 崩溃报告完全不可用 | C++ |
| **P1** | 添加 rate limit 响应头 | 高频场景 SDK 不限速 | 全部 |
| **P2** | `feedback`、`raw_security` 加入 KnownItemType | 数据可查询 | JS |
| **P3** | `sent_at` 时钟漂移校正 | 事件时间精度 | 全部 |
| **P3** | 解析汇总 client_report | 运维可见性 | 全部 |
